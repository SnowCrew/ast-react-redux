  ...
  switch (action.type) {
    ...
    case "ITEM_ADD_TO_CART":
      ...
      if (itemInd >= 0) {
        ...
        const newElem = {
          ...elemInState,
          counterIdentical:
            Number(elemInState?.counterIdentical) + Number(quantity),
        };
        return {
          ...state,
          itemsInCart: [
          ...
          total: Number(
            (
              Number(state.total) +
              Number(newElem.price) * Number(quantity)
            ).toFixed(2)
          ),
      ...
      return {
        ...
        total: Number(
          (state.total + Number(newItem.price) * Number(quantity)).toFixed(2)
        ),
        totalQuantity: Number(state.totalQuantity) + Number(quantity),
      };
    ...
    case "ITEM_REMOVE_FROM_CART":
      ...
      return {
        ...
        total: Number(
          (state.total - itemDel!.price * itemDel!.counterIdentical).toFixed(2)
        ),
